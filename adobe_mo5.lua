...
function linearPalette.ditherCache:save()
	local f = io.open(".adobe_mo5.dc" .. MAXCOL .. "_" .. dx*dy, "wb")
	if f then
...
function linearPalette.ditherCache:load()
	local f = io.open(".adobe_mo5.dc" .. MAXCOL .. "_" .. dx*dy, "rb")
	if f then
		while true do
			local k = f:read(3) 
...
function best_couple:save()
	local f = io.open(".adobe_mo5.bc" .. MAXCOL .. "_" .. dx*dy, "wb")
	if f then
		local function w(i)
...
function best_couple:load()
	local f = io.open(".adobe_mo5.bc" .. MAXCOL .. "_" .. dx*dy, "rb")
	if f then
		local function r()
			local i,s = 0
			repeat
