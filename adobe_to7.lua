...
function linearPalette.ditherCache:save()
	local f = io.open(".adobe_to7.dc" .. MAXCOL .. "_" .. dx*dy, "wb")
	if f then
		for k,v in pairs(self) do 
			if type(v)=="string" then f:write(k,v) end 
		end
		f:close()
	end
end
function linearPalette.ditherCache:load()
	local f = io.open(".adobe_to7.dc" .. MAXCOL .. "_" .. dx*dy, "rb")
	if f then
		while true do
			local k = f:read(3) 
			if k==nil then break end
			local v = f:read(dx*dy)
...
function best_couple:save()
	local f = io.open(".adobe_to7.bc" .. MAXCOL .. "_" .. dx*dy, "wb")
	if f then
		local function w(i)
			local t = string.char(i%128)
			while i>=128 do
				i = math.floor(i/128)
				t = string.char((i%128) + 128) .. t
			end
			return t
		end	
		for k,v in pairs(self.cache) do if type(k)=="number" then f:write(w(k),w(v.c1*16 + v.c2 - 17)) end end
		f:close()
	end
end
function best_couple:load()
	local f = io.open(".adobe_to7.bc" .. MAXCOL .. "_" .. dx*dy, "rb")
	if f then
		local function r()
			local i,s = 0
			repeat
		...
		while true do
			local k = r()   if k==nil then break end
			local c = r()
			self.cache[k] = {c1=1+math.floor(c/16), c2=1+(c%16)}
			self.n = self.n + 1
		end
		f:close()
	end
...
-- convert picture
linearPalette.ditherCache:load()
best_couple:load()
local err = {}
...
best_couple:save()
linearPalette.ditherCache:save()
-- refresh screen
